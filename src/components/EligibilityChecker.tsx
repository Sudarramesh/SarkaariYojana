...
export const EligibilityChecker: React.FC<EligibilityCheckerProps> = ({
  ...
  const occupationsTranslations: Record<string, string> = {
    'Farmer': currentLanguage === 'en' ? 'Farmer' : 'किसान',
    'Student': currentLanguage === 'en' ? 'Student' : 'छात्र/छात्रा',
    'Self-employed': currentLanguage === 'en' ? 'Self-employed' : 'स्व-नियोजित',
    'Salaried': currentLanguage === 'en' ? 'Salaried' : 'वेतनभोगी',
    'Unemployed': currentLanguage === 'en' ? 'Unemployed' : 'बेरोजगार',
  };

  const incomeRanges = [
    { label: currentLanguage === 'en' ? 'Below ₹1.5 Lakhs' : '₹1.5 लाख से कम', maxNumeric: 150000, value: 'Below ₹1.5 Lakhs' },
    { label: currentLanguage === 'en' ? '₹1.5–3 Lakhs' : '₹1.5–3 लाख', maxNumeric: 300000, value: '₹1.5–3 Lakhs' },
    { label: currentLanguage === 'en' ? '₹3–6 Lakhs' : '₹3–6 लाख', maxNumeric: 600000, value: '₹3–6 Lakhs' },
    { label: currentLanguage === 'en' ? '₹6–10 Lakhs' : '₹6–10 लाख', maxNumeric: 1000000, value: '₹6–10 Lakhs' },
    { label: currentLanguage === 'en' ? 'Above ₹10 Lakhs' : '₹10 लाख से अधिक', maxNumeric: 99999999, value: 'Above ₹10 Lakhs' }
  ];

  const handleCheck = (e: React.FormEvent) => {
    e.preventDefault();
    if (!age || !incomeRange || !state || !occupation) {
      alert(currentLanguage === 'en' ? 'Please fill in all general fields to analyze' : 'विश्लेषण करने के लिए कृपया सभी फ़ील्ड भरें');
      return;
    }
          ...
          {hasChecked ? (
            <div className="space-y-5 text-left">
              ...
              {matches.length > 0 ? (
                <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                  {matches.map((scheme) => {
                    const mappedName = currentLanguage === 'hi' && scheme.hiName ? scheme.hiName : scheme.name;
                    const mappedCategory = currentLanguage === 'hi' && scheme.category === 'Agriculture' ? 'कृषि' : scheme.category;
                    const mappedBenefitsLabel = currentLanguage === 'hi' && scheme.hiBenefitsLabel ? scheme.hiBenefitsLabel : scheme.benefitsLabel;
                      ...
                      >
                        <div className="space-y-1 text-left">
                          ...
                          <span className="text-[10px] font-bold text-green-india block">
                            {currentLanguage === 'en' ? 'Benefit: ' : 'लाभ: '}{mappedBenefitsLabel}
                          </span>
                        </div>
                        <div className="flex gap-1.5 shrink-0">
                          <button
                            ...
                            className="px-2.5 py-1.5 rounded border border-slate-200 text-slate-600 text-[10px] font-bold bg-white hover:bg-slate-50 cursor-pointer"
                          >
                            {t.guideBtn}
                          </button>
                          <button
                          ...
                          >
                            {currentLanguage === 'en' ? 'Apply' : 'आवेदन करें'}
                          </button>
                        </div>
                      </div>
                    );
            ...
            <form onSubmit={handleCheck} className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
              ...
              <div className="flex flex-col gap-1 text-left">
                ...
                >
                  <option value="" className="text-slate-800">
                    {currentLanguage === 'en' ? 'Select Occupation' : 'व्यवसाय चुनें'}
                  </option>
