...
export const ApplyModal: React.FC<ApplyModalProps> = ({
  ...
  const validate = () => {
    ...
    if (!name.trim()) errs.name = isEn ? 'Full name is required' : 'पूरा नाम अनिवार्य है';
    if (!age || Number(age) <= 0 || Number(age) > 120) errs.age = isEn ? 'Provide a valid age (1-120)' : 'एक वैध आयु प्रदान करें (1-120)';
    if (!phone || phone.length !== 10 || isNaN(Number(phone))) errs.phone = isEn ? '10-digit mobile number required' : '10 अंकों का मोबाइल नंबर अनिवार्य है';
    if (!aadhaar || aadhaar.length !== 12 || isNaN(Number(aadhaar))) errs.aadhaar = isEn ? '12-digit Aadhaar number is mandatory' : '12 अंकों का आधार नंबर अनिवार्य है';
    if (!income || Number(income) < 0) errs.income = isEn ? 'Provide annual household income' : 'वार्षिक घरेलू आय प्रदान करें';
    if (!declared) errs.declared = isEn ? 'Please agree to the system declarations' : 'कृपया सिस्टम घोषणाओं से सहमत हों';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const formattedDate = currentDate.toLocaleDateString(isEn ? 'en-GB' : 'hi-IN', {
      day: 'numeric',
      month: 'long',
      year: 'numeric'
    });
    const formattedTime = currentDate.toLocaleTimeString(isEn ? 'en-US' : 'hi-IN', {
      hour: '2-digit',
      minute: '2-digit'
    });
    ...
    const displayName = isEn ? scheme.name : (scheme.hiName || scheme.name);
    const displayCategory = isEn ? scheme.category : (scheme.category === 'Agriculture' ? 'कृषि' : scheme.category === 'Health' ? 'स्वास्थ्य' : scheme.category === 'Education' ? 'शिक्षा' : scheme.category === 'Housing' ? 'आवास' : scheme.category === 'Women' ? 'महिला कल्याण' : 'केंद्रीय योजना');
    ...
    const newApp: Application = {
      ...
      timeline: [
        {
          status: isEn ? 'Application Submitted' : 'आवेदन जमा हुआ',
        ...
        {
          status: isEn ? 'Document Verification' : 'दस्तावेज सत्यापन',
          date: isEn ? 'Processing' : 'प्रक्रिया जारी',
          description: isEn
        ...
        {
          status: isEn ? 'Approval Granted' : 'स्वीकृति दी गई',
          date: isEn ? 'Pending' : 'लंबित',
          description: isEn
        ...
        {
          status: isEn ? 'Payment Processing' : 'भुगतान प्रक्रिया',
          date: isEn ? 'Pending' : 'लंबित',
          description: isEn
  ...
  const handleCopyCode = () => {
    if (successApp) {
      ...
      alert(isEn ? 'Tracking Reference copied to clipboard!' : 'ट्रैकिंग संदर्भ क्लिपबोर्ड पर कॉपी किया गया!');
    }
  };
