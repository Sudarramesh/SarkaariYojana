...
export const SchemeDetailsModal: React.FC<SchemeDetailsModalProps> = ({
  ...
  const displaySponsor = currentLanguage === 'hi' && scheme.hiSponsor ? scheme.hiSponsor : scheme.sponsor;
  const displayDesc = currentLanguage === 'hi' && scheme.hiDescription ? scheme.hiDescription : scheme.description;
  const displayBenefits = currentLanguage === 'hi' && scheme.hiBenefits ? scheme.hiBenefits : scheme.benefits;
  const displayDeadline = currentLanguage === 'hi'
    ? (scheme.hiTimelineLabel || scheme.hiDeadline || 'सतत आवेदन')
    : (scheme.deadline || 'Ongoing Application');
  const displayKeyPoints = currentLanguage === 'hi' && scheme.hiKeyPoints ? scheme.hiKeyPoints : scheme.keyPoints;
  const displayDocuments = currentLanguage === 'hi' && scheme.hiDocumentsRequired ? scheme.hiDocumentsRequired : scheme.documentsRequired;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl relative z-10 overflow-hidden border border-slate-100 flex flex-col my-8 font-sans">
          ...
          <div className="flex flex-col gap-1.5 text-left">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-[11px] font-medium text-slate-400">
                {currentLanguage === 'en' ? 'Sponsored by: ' : 'द्वारा प्रायोजित: '} {displaySponsor}
              </span>
            </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 max-h-[70vh]">
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-150">
                  <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400 block">
                    {currentLanguage === 'en' ? 'Benefits' : 'योग्य लाभ'}
                  </span>
                ...
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-150">
                  <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400 block">
                    {currentLanguage === 'en' ? 'Status / Deadline' : 'आवेदन समय सीमा'}
                  </span>
                  <span className="text-sm font-bold text-[#001b44] flex items-center gap-1.5 mt-0.5">
          ...
          <div className="space-y-3 text-left">
            <h4 className="text-sm font-extrabold text-[#001b44] uppercase tracking-wide flex items-center gap-2">
              ...
              {currentLanguage === 'en' ? 'Key Highlights' : 'मुख्य विशेषताएं'}
            </h4>
          ...
          <div className="bg-slate-50/70 rounded-2xl p-5 border border-slate-100 space-y-3 text-left">
            <h4 className="text-sm font-extrabold text-[#001b44] uppercase tracking-wide flex items-center gap-2">
              ...
              {currentLanguage === 'en' ? 'Mandatory Documents Checklist' : 'अनिवार्य दस्तावेज चेकलिस्ट'}
            </h4>
          ...
          <div className="p-4 bg-orange-50 rounded-xl border border-orange-100 flex gap-3 text-xs text-orange-850 text-left">
            ...
            <div className="space-y-1 text-left">
              <span className="font-bold block text-left">
                {currentLanguage === 'en' ? 'Eligibility Threshold Warning' : 'पात्रता सीमा चेतावनी'}
              </span>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-3 justify-end">
          ...
          >
            {currentLanguage === 'en' ? 'Close Guide' : 'गाइड बंद करें'}
          </button>
          <button
            onClick={() => {
              onApply(scheme.id);
          ...
          >
            {currentLanguage === 'en' ? 'Apply Online' : 'ऑनलाइन आवेदन करें'}
          </button>
        </div>
      </div>
    </div>
